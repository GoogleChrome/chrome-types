...
Options:
  ...
  -s, --symbols        path to symbol history file from prepare-history.js
      --root           chrome or browser (default: chrome)
`);
    process.exit(0);
  }

  if (!roots.includes(argv.root)) {
    throw new Error(`--root must be one of: ${roots.join(', ')}`);
  }
  if (argv.all && argv.root !== 'chrome') {
    // Platform Apps never had a browser global, so we only build that bundle under chrome.
    throw new Error('--all is only available with --root=chrome');
  }
