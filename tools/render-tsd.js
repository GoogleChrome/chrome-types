...
import { renderNamespaceBundle, roots } from './lib/render-bundle.js';
...
Options:
  ...
  -s, --symbols        path to symbol history file from prepare-history.js
      --root           chrome or browser (default: chrome); browser also gets a chrome
                       namespace re-exporting every API, and excludes --all
`);
    process.exit(0);
  }

  if (!roots.includes(argv.root)) {
    console.warn(`--root must be one of: ${roots.join(', ')}`);
    process.exit(1);
  }
  if (argv.all && argv.root !== 'chrome') {
    // Platform Apps have no browser global, so the all bundle only exists under chrome.
    console.warn('--all is only available with --root=chrome');
    process.exit(1);
  }
...
// Built at ${o.definitionsRevision}${versionSuffix}`);
  ...
  renderParts.push(...renderNamespaceBundle(Object.values(o.api), renderContext, {
    root: argv.root,
  }));
