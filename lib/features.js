...
function filterOtherRow(row) {
  const {command_line_switch, allowlist} = row;
  if (allowlist?.length || command_line_switch) {
    return false;
  }
...
export function extractConfig(id, features) {
  ...
  // Filter rows. Notably we _don't_ filter `internal: true`, as this masks a bunch of real APIs.
