...
export function fullName(reflection) {
  ...
  while (r && r.kind !== rk.Module && r.kind !== rk.Project) {
    ...
    if (r.name.startsWith('__')) {
      ...
      // We can get the "__type" magical declaration here, and we actually need to go up to find
      // its actual reflection and find its name if possible.
      if (r.parent instanceof typedocModels.DeclarationReflection) {
        const actualType = r.parent.children?.find((child) => {
          if (child.type instanceof typedocModels.ReflectionType) {
            return child.type.declaration === r;
          }
        });

        if (actualType) {
          parts.unshift(actualType.name);
          found = true;
        }
      }
