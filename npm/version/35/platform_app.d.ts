 ...
 associated
  ...
  export namespace app.window {
    ...
    export interface FrameOptions {
       ...
       * frame type is <code>chrome</code>. Frame coloring is experimental and only
    ...
    export interface CreateWindowOptions {
       ...
       * <code>outerBounds</code> will result in an error.</p><p>Currently only
       * available on the Dev channel from Chrome 35.</p>
       */
       ...
       * error.</p><p>Currently only available on the Dev channel from Chrome 35.</p>
       */
       ...
       * Requires the <code>"app.window.alwaysOnTop"</code> permission. Defaults to
    ...
    export interface AppWindow {
       ...
       * ESC is pressed by requesting the <b>app.window.fullscreen.overrideEsc</b>
       * permission and canceling the event by calling .preventDefault(), like
       * this:<br> <code>window.onKeyDown = function(e) { if (e.keyCode == 27 /* ESC
       ...
       * include window decorations. Currently only available on the Dev channel from
       * Chrome 35.
       */
      innerBounds: Bounds;
       ...
       * decorations, such as the title bar and frame. Currently only available on the
       * Dev channel from Chrome 35.
       */
      outerBounds: Bounds;
    }
     ...
     * used.</p><p>Another option is to use the <code>bounds</code> property, which
     * will put the window at the specified coordinates with the specified size. If
     * the window has a frame, it's total size will be the size given plus the size
     * of the frame; that is, the size in bounds is the content size, not the window
     * size.</p><p>To automatically remember the positions of windows you can give
     * them ids. If a window has an id, This id is used to remember the size and
     * position of the window whenever it is moved or resized. This size and
     ...
     * location other than the remembered default, you can create it hidden, move it
     * to the desired location, then show it.</p>
     * @param url
     * @param options
 ...
 */
  export namespace bluetooth {
    ...
    /**
     * Read data from a Bluetooth connection. The |callback| will be called with the
     * current data in the buffer even if it is empty. This function should be
     * polled to read incoming data.
     * @param options The options for this function.
     * @param callback Called with the data read from the socket buffer.
     * @param callback.result
     */
     ...
     * @param options The options for this function.
     * @param callback Called with the number of bytes written.
     * @param callback.result
     */
 ...
 */
  export namespace diagnostics {
    ...
    export interface SendPacketResult {
      /**
       * The IP of the host which we recieves the ICMP reply from. The IP may differs
 ...
 */
  export namespace fileSystemProvider {
     ...
     * be an empty string. <code>displayName</code> should be descritive but doesn't
     * have to be unique. Duplicate display names are uniquified by adding suffix
     * like "(1)" in the Files.app UI.
     * @param displayName
 ...
 */
  export namespace webviewTag {
    ...
    export interface ClearDataTypeSet {
      ...
      /**
       * The partition's cache. Note: This clears the entire cache regardless of the
       * age passed to <code>clearData</code>.
       */
      cache?: boolean;
      ...
      /**
       * The partition's stored form data.
       */
      formData?: boolean;
     ...
     * red:</p><pre>webview.executeScript({ code: "document.body.bgColor = 'red'"
     * });</pre>
     * @param details Details of the script to run.
     * @param callback Called after all the JavaScript has been executed.
     * @param callback.result The result of the script in every injected frame.
