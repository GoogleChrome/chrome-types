...
// Generated on Thu Dec 17 2020 17:36:14 GMT+1100 (Australian Eastern Daylight Time)
 ...
 */
  export namespace automation {
     ...
     */
    export type EventCommandType = "clearSelection" | "cut" | "delete" | "dictate" | "extendSelection" | "format" | "insert" | "marker" | "moveSelection" | "paste" | "replace" | "setSelection" | "type";

    /**
     * A text boundary associated with an AutomationEvent.
     */
    export type EventTextBoundaryType = "character" | "format" | "lineEnd" | "lineStart" | "lineStartOrEnd" | "object" | "pageEnd" | "pageStart" | "pageStartOrEnd" | "paragraphEnd" | "paragraphStart" | "paragraphStartOrEnd" | "sentenceEnd" | "sentenceStart" | "sentenceStartOrEnd" | "webPage" | "wordEnd" | "wordStart" | "wordStartOrEnd";

    /**
     * A move direction associated with an AutomationEvent.
     */
    export type EventMoveDirectionType = "forward" | "backward";
 ...
 */
  export namespace declarativeNetRequest {
     ...
     * listed in <code>ruleIdsToRemove</code> are first removed, and then the rules
     * given in <code>rulesToAdd</code> are added. This update happens as a single
     * atomic operation: either all specified rules are added and removed, or an
     * error is returned. These rules are persisted across browser sessions and
     * across extension updates. Any ids in <code>ruleIdsToRemove</code> that are
     * not present will be ignored. Note that static rules specified as part of the
     * extension package can not be removed using this function. Note: {@link
     * MAX_NUMBER_OF_DYNAMIC_RULES} is the maximum number of dynamic rules an
     * extension can add.
     * @param ruleIdsToRemove The IDs of rules to remove.
     ...
     * with IDs listed in <code>rulesetIDsToDisable</code> are first removed, and
     * then the rulesets listed in <code>rulesetIDsToEnable</code> are added. Note
     * that the set of enabled static rulesets is persisted across sessions but not
     * across extension updates, i.e. the <code>rule_resources</code> manifest key
     * will determine the set of enabled static rulesets on each extension update.
     * @param rulesetIdsToDisable The set of ids corresponding to a static {@link Ruleset} that should be disabled.
     * @param rulesetIdsToEnable The set of ids corresponding to a static {@link Ruleset} that should be enabled.
    ...
    export function updateEnabledRulesets(
      rulesetIdsToDisable: string[],
      rulesetIdsToEnable: string[],
      callback: () => void,
    ): void;
