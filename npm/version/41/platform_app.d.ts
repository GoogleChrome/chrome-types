...
// Generated on Thu Dec 17 2020 17:49:40 GMT+1100 (Australian Eastern Daylight Time)
...
declare namespace chrome {
   ...
   * portals.
   */
  export namespace networking.captivePortal {
    /**
    ...
    export interface WiFiFilter {
      /**
       * List of SSIDs that can be handled by the extension. Note that the entries of
       * <code>SSID</code> can only represent Unicode strings. SSID will be used in
       * the UTF-8 encoding. For arbitrary byte sequences use <code>HexSSID</code>.
       */
      SSID: string[];
     ...
     */
    export type AuthenticationResult = "unhandled" | "success" | "rejected" | "failed";
     ...
     * supplied filters.
     * @param networkFilter network filter to add
     */
    export function addNetworkFilter(
     ...
     */
    export function removeAllNetworkFilters(): void;

    /**
     * Called by the extension to notify the captive portal API that it finished an
     * authentication attempt and hand over the result of the attempt. This function
     * must only be called with the GUID of the latest $(onCaptivePortalDetected)
     * event.
     ...
     * does not fires. Upon receiving this event the extension should start its
     * authentication attempt with the captive portal. When the extension
     * finishes its attempt it must call $(finishAuthentication) with the
     * <code>GUID</code> received with this event and the appropriate
     * $(AuthenticationResult) to notify the captive portal API.
     */
    export const onCaptivePortalDetected: chrome.events.Event<
      /**
       * @param networkInfo detailed information about the network on which a captive                portal was detected.
       */
      (
 ...
 */
  export namespace copresenceEndpoints {
    export interface EndpointInfo {
      ...
      /**
       * An opaque string containing the locator data for this endpoint. This locator
       * is needed to connect to this endpoint.
       */
      locator: string;
    }
     ...
     */
    export type EndpointStatus = "no_error" | "disconnected" | "invalid_local_endpoint" | "invalid_remote_endpoint" | "connect_failure" | "send_failure" | "receive_failure";

    /**
     * Creates a endpoint that can be connected to by a nearby devices.
     * @param callback Called when the endpoint has been created.
     * @param callback.endpointInfo The result of the endpoint creation.
     */
    ...
    /**
     * Destroys the endpoint. This will close any connections to this endpoint from
     * remote hosts and will prevent any further connections to it.
     * @param endpointId Endpoint ID returned by <code>createEndpoint</code>.
     */
    export function destroyEndpoint(
      endpointId: number,
    ): void;

    /**
     * Sends data from a local Copresence endpoint to a remote endpoint.
     * @param localEndpointId The local endpoint identifier.
     * @param remoteEndpointId The remote endpoint identifier.
     * @param data The data to send.
    ...
    export const onConnected: chrome.events.Event<
      /**
       * @param localEndpointId ID of the local endpoint that received this                     connection. TODO(rkc): This API needs to be modified to give a valid remote endpoint id also. This currently doesn't happen because of the lack of a handshake on a sockets connection. Once we have a handshake, modify this API to also return a remote endpoint id.
       */
      (
        localEndpointId: number,
      ) => void
 ...
 */
  export namespace vpnProvider {
    export interface Parameters {
      ...
      /**
       * Bypass network traffic to the below IPs from the tunnel. Typically used to
       * bypass traffic to and from the VPN server. Currently only one IP is
       * supported.
       */
